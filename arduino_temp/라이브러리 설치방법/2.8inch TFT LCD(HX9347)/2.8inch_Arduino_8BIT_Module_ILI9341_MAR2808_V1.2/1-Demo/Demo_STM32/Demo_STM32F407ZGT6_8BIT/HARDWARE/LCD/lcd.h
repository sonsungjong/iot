...
#define	LCD_WR_SET	LCD_WR=1  //GPIO_TYPE->BSRRL=1<<WR         //写
#define	LCD_RD_SET	LCD_RD=1  //GPIO_TYPE->BSRRL=1<<RD         //读
...
#define	LCD_WR_CLR	LCD_WR=0  //GPIO_TYPE->BSRRH=1<<WR         //写
#define	LCD_RD_CLR	LCD_RD=0  //GPIO_TYPE->BSRRH=1<<RD         //读
				
//PF0~15,作为数据线
//注意：如果使用8位模式数据总线，则液晶屏的数据高8位是接到MCU的高8位总线上
//举例：如果接8位模式则本示例接线为液晶屏DB10-DB17对应接至单片机GPIOF_Pin8-GPIOF_Pin15
//举例：如果是16位模式：DB0-DB7分别接GPIOF_Pin0-GPIOF_Pin7,DB10-DB17对应接至单片机GPIOF_Pin8-GPIOF_Pin15
#define DATAOUT(x) GPIOG->ODR=x; //数据输出
#define DATAIN     GPIOG->IDR;   //数据输入
